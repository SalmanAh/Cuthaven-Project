...
export const Route = createFileRoute("/account/reset-password")({
  head: () => ({ meta: [
    ...
    { property: "og:title", content: "Create New Password — CutHaven" },
    { property: "og:description", content: "Set a new password for your CutHaven account." },
  ] }),
      ...
      <div className="bg-surface rounded-2xl border border-border shadow-sm p-8">
        ...
        {done ? (
          <div className="text-center py-4">
            ...
            <Link to="/account/login" className="btn-primary mt-6 inline-flex">Sign In</Link>
          </div>
        ) : (
          <form className="space-y-4" onSubmit={submit}>
            <div>
              <label htmlFor="rp-new" className="block text-sm font-medium mb-1.5">New Password</label>
              <div className="relative">
                <input id="rp-new" type={show1 ? "text" : "password"} autoComplete="new-password" required value={pw}
                ...
                <button type="button" onClick={() => setShow1(!show1)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary" aria-label="Toggle password">
            ...
            <div>
              <label htmlFor="rp-conf" className="block text-sm font-medium mb-1.5">Confirm New Password</label>
              <div className="relative">
                <input id="rp-conf" type={show2 ? "text" : "password"} autoComplete="new-password" required value={confirm}
                ...
                <button type="button" onClick={() => setShow2(!show2)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary" aria-label="Toggle password">
