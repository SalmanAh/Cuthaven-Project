  ...
  return (
    <div className="group card-surface overflow-hidden flex flex-col">
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        {product.salePrice && (
          <span className="absolute top-3 left-3 bg-destructive text-white text-[10px] font-bold px-2.5 py-1 rounded-full">SALE</span>
        )}
        {!product.inStock && (
          <span className="absolute top-3 left-3 bg-black/70 text-white text-[10px] font-bold px-2.5 py-1 rounded-full">OUT OF STOCK</span>
        )}
      ...
      <div className="p-4 flex flex-col gap-1.5 flex-1">
        <p className="text-[11px] uppercase tracking-wide text-text-secondary">{product.category}</p>
        <Link to="/product/$slug" params={{ slug: product.slug }}
        ...
        <div className="flex items-center gap-1 text-xs text-text-secondary">
          <div className="flex text-warning">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`h-3 w-3 ${i < Math.round(product.rating) ? "fill-current" : ""}`} />
            ))}
          </div>
        ...
        <div className="flex items-baseline gap-2 mt-1">
          ...
          {product.salePrice && <span className="text-xs text-text-muted line-through">${product.price.toFixed(2)}</span>}
        </div>
        <div className="mt-3 flex gap-2">
          <button disabled={!product.inStock} onClick={() => addItem(product)}
