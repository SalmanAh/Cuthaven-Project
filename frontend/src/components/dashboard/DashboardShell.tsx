...
export function DashboardShell({
  title, subtitle, sidebarTitle, nav, activeKey, onSelect, children,
}: {
  title: string; subtitle?: string; sidebarTitle: string;
  nav: NavItem[]; activeKey: string; onSelect: (key: string) => void; children: ReactNode;
}) {
    ...
    <div className="bg-background min-h-screen">
      <div className="mx-auto max-w-7xl px-4 py-6 md:py-10 grid md:grid-cols-[240px_1fr] gap-6">
        <aside className="hidden md:block">
          <div className="bg-surface rounded-2xl border border-border shadow-sm p-4 sticky top-24">
            <p className="px-3 text-xs uppercase tracking-wide text-text-secondary font-semibold mb-3">{sidebarTitle}</p>
            <nav className="space-y-1">
              {nav.map((n) => (
                <button key={n.key} onClick={() => n.onClick ? n.onClick() : onSelect(n.key)}
        ...
        <div className="min-w-0">
          <div className="md:hidden mb-4 flex items-center justify-between">
            ...
            <button onClick={() => setMobileOpen(true)} aria-label="Open menu" className="p-2 rounded-lg border border-border"><Menu className="h-5 w-5" /></button>
          </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 md:hidden" onClick={() => setMobileOpen(false)}>
          <aside className="bg-surface h-full w-[80%] max-w-xs p-5 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <nav className="space-y-1">
              {nav.map((n) => (
                <button key={n.key} onClick={() => { n.onClick ? n.onClick() : onSelect(n.key); setMobileOpen(false); }}
