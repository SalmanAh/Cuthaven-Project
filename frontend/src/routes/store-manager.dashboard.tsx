          ...
          {statuses.map((s) => {
            ...
            return (
              <button
                key={s}
                onClick={() => setFilter(s)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize ${filter === s ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-primary/10"}`}
              >
                {s} ({count})
              </button>
            );
            ...
            <tbody>
              {products.map((p) => (
                <tr key={p.id} className="border-b border-border">
                  ...
                  <td className="py-3">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs ${p.isActive ? "bg-success/10 text-success" : "bg-muted text-text-secondary"}`}
                    >
