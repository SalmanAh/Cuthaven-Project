...
export const Route = createFileRoute("/contact-us")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contact CutHaven. Reach our team by email, phone, or the contact form. FAQs answered." },
...
const faqs = [
  { q: "How long does shipping take?", a: "Standard shipping takes 5–8 business days across the US. Express options available at checkout." },
  { q: "What is your return policy?", a: "We offer a 40-day hassle-free return policy on all items. Items must be unused and in original packaging." },
  { q: "Do you offer free shipping?", a: "Yes! We offer free shipping on all orders over $350." },
  { q: "How do I track my order?", a: "Use our Track Your Order page with your order number and email address." },
  { q: "Can I change or cancel my order?", a: "Orders can be modified or cancelled within 24 hours of placement. Contact support immediately." },
  { q: "Are your products covered by warranty?", a: "All products include a 12-month manufacturer warranty against defects." },
  { q: "What payment methods do you accept?", a: "We accept Visa, Mastercard, PayPal, American Express, and Discover." },
  { q: "Do you ship internationally?", a: "Currently we ship within the United States only." },
  { q: "How do I contact customer support?", a: "Email support@cuthaven.com or call +1 (406) 229-9045." },
  { q: "Are your products authentic?", a: "All products are 100% authentic and sourced directly from manufacturers." },
];
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 grid md:grid-cols-3 gap-5">
        {[
          { icon: MapPin, title: "Our Location", lines: ["1633 S Industrial Way", "Palmer, AK 99645"] },
          { icon: Phone, title: "Call Us", lines: ["+1 (406) 229-9045"] },
          { icon: Mail, title: "Email Us", lines: ["support@cuthaven.com", "We reply within 24 hours"] },
        ].map((c) => (
          <div key={c.title} className="card-surface p-6 text-center">
            <div className="h-12 w-12 rounded-full bg-primary/10 grid place-items-center mx-auto mb-3"><c.icon className="h-5 w-5 text-primary" /></div>
            <p className="font-semibold mb-1">{c.title}</p>
            {c.lines.map((l) => <p key={l} className="text-sm text-text-secondary">{l}</p>)}
          </div>
        ))}
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-14 grid md:grid-cols-2 gap-8">
        ...
        <div className="rounded-2xl overflow-hidden border border-border shadow-sm">
          ...
          <div className="p-6">
            ...
            <p className="text-sm text-text-secondary">Mon–Fri: 9am–6pm EST<br />Sat: 10am–4pm EST<br />Sun: Closed</p>
          </div>
        </div>
      </section>
