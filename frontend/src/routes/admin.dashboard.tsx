  ...
  return (
    <div className="space-y-4 sm:space-y-5 md:space-y-6">
      <div className="flex justify-end">
        <TimeFilter value={period} onChange={(p) => setPeriod(p as AdminPeriod)} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <DashCard>
          <h3 className="font-display text-base sm:text-lg font-bold mb-3 sm:mb-4">
            Orders by Status
          </h3>
          <div className="h-48 sm:h-56 md:h-64">
            <ResponsiveContainer>
      ...
      <DashCard>
          ...
          {statuses.map((s) => {
            ...
            return (
              <button
                key={s}
                onClick={() => setFilter(s)}
                className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-medium capitalize ${filter === s ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-primary/10"}`}
              >
                {s} ({count})
              </button>
            );
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 items-center mb-3 sm:mb-4">
          <div className="relative flex-1 min-w-[160px] sm:min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-8 sm:pl-9 pr-2 sm:pr-3 py-1.5 sm:py-2 rounded-lg border border-border text-xs sm:text-sm focus:outline-none focus:border-primary"
            />
          </div>
          <select
            value={payFilter}
            onChange={(e) => setPayFilter(e.target.value)}
            className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-border text-xs sm:text-sm bg-surface"
          >
            <option>All</option>
            <option>Paid</option>
        ...
        {loading ? (
          ...
          <>
            <div className="overflow-x-auto -mx-4 sm:mx-0">
              <div className="min-w-[640px] px-4 sm:px-0">
                <table className="w-full text-xs sm:text-sm">
                  <thead>
                    <tr className="text-left text-text-secondary border-b border-border">
                      <th className="py-2 pr-2 sm:pr-3">Order #</th>
                      <th className="py-2 pr-2 sm:pr-3">Customer</th>
                      <th className="py-2 pr-2 sm:pr-3">Date</th>
                      <th className="py-2 pr-2 sm:pr-3">Status</th>
                      <th className="py-2 pr-2 sm:pr-3">Payment</th>
                      <th className="py-2 pr-2 sm:pr-3">Total</th>
                  ...
                  <tbody>
                    {orders.map((o) => (
                      <tr key={o.id} className="border-b border-border">
                        <td className="py-2.5 sm:py-3 pr-2 sm:pr-3 font-mono text-[10px] sm:text-xs">
                          {o.orderNumber}
                        </td>
                        <td className="py-2.5 sm:py-3 pr-2 sm:pr-3 max-w-[120px] truncate">
                        ...
                        <td className="py-2.5 sm:py-3 pr-2 sm:pr-3 font-semibold">
                          ${o.total.toFixed(2)}
                        </td>
                        <td className="py-2.5 sm:py-3">
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => setDetail(o)}
                              className="p-1 sm:p-1.5 hover:bg-muted rounded touch-manipulation"
                              aria-label="View"
                            >
                            ...
                            <select
                              value={o.status}
                              onChange={(e) =>
                                updateStatus(o.id, e.target.value as AdminOrder["status"])
                              }
                              className="text-[10px] sm:text-xs border border-border rounded px-1 sm:px-1.5 py-0.5 sm:py-1 bg-surface"
                            >
                              {[
                                "pending",
                                "confirmed",
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <button className="p-1 sm:p-1.5 hover:bg-muted rounded touch-manipulation">
            ...
            <p className="text-[10px] sm:text-xs text-text-secondary mt-3 sm:mt-4">
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="space-y-2">
                  {detail.items.map((it) => (
                    ...
                    >
                      {it.productImage && (
                        <img src={it.productImage} alt="" className="h-12 w-12 rounded" />
                      )}
                ...
                <div className="pt-3 border-t border-border">
                  ...
                  <p className="text-text-secondary">
                    ...
                    {detail.shippingAddress.state} {detail.shippingAddress.zip}
                  </p>
                </div>
                <p className="text-text-secondary">
                  Payment: {detail.paymentMethod ?? detail.paymentProcessor}
                </p>
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(v) => !v && setToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                setOrders((p) => p.filter((o) => o.id !== toDelete));
...
function ProductsPage() {
  ...
  const catNames = [
    "All",
    ...Array.from(new Set(items.map((p) => p.categoryName).filter(Boolean) as string[])),
  ];
      ...
      <DashCard>
        <div className="flex flex-wrap gap-2 sm:gap-3 items-center justify-between mb-3 sm:mb-4">
          ...
          <button
            ...
            className="btn-primary text-xs sm:text-sm flex items-center gap-1 min-h-[44px] sm:min-h-0 touch-manipulation"
          >
          ...
          <div className="overflow-x-auto -mx-4 sm:mx-0">
            <div className="min-w-[720px] px-4 sm:px-0">
              <table className="w-full text-xs sm:text-sm">
                <thead>
                  <tr className="text-left text-text-secondary border-b border-border">
                    <th className="py-2 pr-2 sm:pr-3">Image</th>
                    <th className="py-2 pr-2 sm:pr-3">Name</th>
                    <th className="py-2 pr-2 sm:pr-3">SKU</th>
                    <th className="py-2 pr-2 sm:pr-3">Category</th>
                    <th className="py-2 pr-2 sm:pr-3">Price</th>
                    <th className="py-2 pr-2 sm:pr-3">Stock</th>
                    <th className="py-2 pr-2 sm:pr-3">Active</th>
                    <th className="py-2">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.id} className="border-b border-border">
                      ...
                      <td className="py-2.5 sm:py-3 pr-2 sm:pr-3 font-medium max-w-[160px] sm:max-w-[220px] truncate">
                        {p.name}
                      </td>
                      <td className="py-2.5 sm:py-3 pr-2 sm:pr-3 font-mono text-[10px] sm:text-xs">
                        {p.sku ?? "—"}
                      ...
                      <td className="py-2.5 sm:py-3 pr-2 sm:pr-3">
                        <Switch
                          ...
                          onCheckedChange={async (v) => {
                            try {
                              await adminUpdateProduct(p.id, { isActive: v });
                              setItems((prev) =>
                      ...
                      <td className="py-2.5 sm:py-3">
                        <div className="flex gap-1">
                          ...
                          <button
                            ...
                            className="p-1 sm:p-1.5 hover:bg-muted rounded text-destructive touch-manipulation"
                          >
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(v) => !v && setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The product will be hidden from the store but order history is preserved.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => toDelete && softDelete(toDelete)}>
              Deactivate
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
    ...
    >
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            await onSave({ ...form, slug: form.slug || genSlug(form.name), id: initial?.id });
          }}
        ...
        >
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 rounded-lg border border-border text-sm bg-surface"
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => upd("compareAtPrice", e.target.value ? +e.target.value : null)}
              className="w-full px-3 py-2 rounded-lg border border-border text-sm"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => upd("stockQuantity", +e.target.value)}
              className="w-full px-3 py-2 rounded-lg border border-border text-sm"
            />
          </div>
          ...
          <div className="col-span-2">
            ...
            <div className="flex gap-2 items-center">
              ...
              />
              <label
                className={`cursor-pointer px-3 py-2 rounded-lg border border-border text-sm font-medium hover:bg-muted transition whitespace-nowrap ${uploadingPrimary ? "opacity-50 pointer-events-none" : ""}`}
              >
          ...
          <div className="col-span-2">
            <label className="block text-xs font-medium mb-1">
              ...
              <span className="text-text-secondary ml-1 font-normal">
                (up to 9 — shown in product gallery)
              </span>
            </label>

            {/* Upload button */}
            <label
              className={`inline-flex items-center gap-1.5 cursor-pointer px-3 py-2 rounded-lg border border-border text-sm font-medium hover:bg-muted transition ${uploadingExtra ? "opacity-50 pointer-events-none" : ""}`}
            >
            ...
            <p className="text-xs text-text-secondary mt-1">
              Select multiple files at once — Ctrl/Cmd+Click to select several.
            </p>
                ...
                <div className="space-y-4 text-sm">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-3 bg-muted rounded-lg">
                      ...
                      <p className="font-bold text-lg">{detail.customer.ordersCount}</p>
                    </div>
                    <div className="p-3 bg-muted rounded-lg">
                      <p className="text-xs text-text-secondary">Total Spent</p>
                      <p className="font-bold text-lg">${detail.customer.totalSpent.toFixed(2)}</p>
                    </div>
                    <div className="p-3 bg-muted rounded-lg">
                      <p className="text-xs text-text-secondary">Joined</p>
                      <p className="font-bold text-base">
                        {detail.customer.createdAt.slice(0, 10)}
                      </p>
                    </div>
                  </div>
    ...
    <div className="space-y-4">
      <DashCard>
        <div className="flex justify-end mb-4">
          ...
          >
            <Plus className="h-4 w-4" /> Add Store Manager
          </button>
        </div>
        {loading ? (
          <Spinner />
                ...
                {staff.map((m) => (
                  <tr key={m.id} className="border-b border-border">
                    ...
                    <td className="py-3 pr-3">
                      <Switch checked={m.isActive} onCheckedChange={(v) => toggle(m.id, v)} />
                    </td>
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(v) => !v && setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove store manager?</AlertDialogTitle>
            <AlertDialogDescription>Their account will be deactivated.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                if (toDelete) {
                  await adminToggleStaff(toDelete, false);
                  setStaff((p) => p.filter((s) => s.id !== toDelete));
    ...
    >
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium mb-1">
              Excerpt * <span className="text-text-secondary">(shown on blog listing)</span>
            </label>
            <textarea
              required
              rows={2}
          ...
          <div>
            <label className="block text-xs font-medium mb-1">
              Content *{" "}
              <span className="text-text-secondary">(separate paragraphs with a blank line)</span>
            </label>
            <textarea
              required
              rows={10}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {r.disclosed_incentive && (
                        <p className="mt-1 text-xs text-warning">
                          ⚠ Reviewer disclosed an incentive
                        </p>
                      )}
                      {r.insider_relationship && (
                        <p className="mt-0.5 text-xs text-warning">
                          ⚠ Insider relationship: {r.insider_relationship}
                        </p>
                      )}
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="flex gap-0.5 items-center">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <Star
                      key={n}
                      className={`h-5 w-5 ${n <= detail.rating ? "fill-warning text-warning" : "text-border"}`}
                    />
                  ))}
                ...
                <div className="space-y-1">
                  ...
                  {detail.disclosed_incentive && (
                    <p className="text-xs text-warning">
                      ⚠ Reviewer disclosed receiving an incentive
                    </p>
                  )}
                  {detail.insider_relationship && (
                    <p className="text-xs text-warning">
                      ⚠ Insider relationship: {detail.insider_relationship}
                    </p>
                  )}
                </div>
              ...
              <tbody>
                {coupons.map((c) => (
                  <tr key={c.id} className="border-b border-border">
                    ...
                    <td className="py-3 pr-3">
                      {c.valid_until
                        ? new Date(c.valid_until).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                            year: "numeric",
                          })
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(v) => !v && setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This cannot be undone. Existing orders that used this coupon are unaffected.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-text-secondary mt-1">
              Customers enter this at checkout. Automatically uppercased.
            </p>
          </div>
          ...
          {form.code && form.discountValue > 0 && (
            <div className="rounded-lg bg-success/10 border border-success/20 px-4 py-3 text-sm">
              ...
              <p className="text-text-secondary mt-0.5">
                ...
                {form.validUntil
                  ? ` · expires ${new Date(form.validUntil).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`
                  : ""}
              </p>
            </div>
          )}
...
function SettingsPage() {
  const [store, setStore] = useState({
    ...
    address: "1633 S Industrial Way, Palmer, AK 99645",
  });
      ...
      <DashCard>
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            toast.success(
              "Saved (UI only — backend settings endpoint coming in a future milestone)",
            );
          }}
      ...
      <DashCard>
        ...
        >
          <div>
            ...
            <input
              type="number"
              value={ship.freeThreshold}
              onChange={(e) => setShip({ ...ship, freeThreshold: +e.target.value })}
              className="w-full px-3 py-2 rounded-lg border border-border text-sm"
            />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1">Flat rate ($)</label>
            <input
              type="number"
              step="0.01"
              value={ship.flatRate}
              onChange={(e) => setShip({ ...ship, flatRate: +e.target.value })}
              className="w-full px-3 py-2 rounded-lg border border-border text-sm"
            />
          </div>
          <div className="md:col-span-2 flex justify-end">
      ...
      <AlertDialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this payment gateway. If it's the only active gateway of
              its type, you cannot delete it.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
function GatewayCard({ gateway, onActivate, onEdit, onDelete }: GatewayCardProps) {
  ...
  return (
    <div
      className={`rounded-lg border ${gateway.isActive ? "border-primary bg-primary/5" : "border-border bg-surface"} p-4`}
    >
